...
```bash
docker volume create cw-vol #volume olusturuyoruz
```
...
```#amac container olustururken volume baglamak -v(volume) alpine(image oluyor)(i-interactive,t=terminal)
...
```bash
sudo ls -al  /var/lib/docker/volumes/cw-vole/_data && sudo cat /var/lib/docker/volumes/cw-vol/_data/i-will-persist.txt
```
...
```bash
docker volume rm cw-vol #docker volume prune -f(force)
```
