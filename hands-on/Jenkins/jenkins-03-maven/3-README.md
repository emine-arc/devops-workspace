...
- At `General Tab`, select Discard old builds, `Strategy`(Discard old builds) is `Log Rotation`, and for `Days to keep builds` enter `5` and `Max # of builds to keep` enter `1`.
...
## Part 6 - Jenkins Job DSL # burda kaldik

- Install `Job DSL` plugin #manage jenkins >>managr plugin >> avaliable sec
...
#Hata alinca console baglaniyoruz.Please tell me who you are.hatasi alinca girecegimiz komutlar sunlardir
...
sudo su - jenkins
    ...
    5  cd workspace
    6  ls
    7  cd Package-Application/
    8  ls
git config --global user.name xxxxxx(github user name)yaz
git config --global user.email xxxxxxx(github kullandigin mail)yaz
