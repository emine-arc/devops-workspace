...
>
> ```bash
> # Swap a usb extern (3.7 GB):  # ram yerine kullanilan alana swap denir
...
# Oncelikle bazi paketler yuklememiz gerekir ve bunlar bizim kubernetesi bulmamiza yardimci olacaklar
```bash
sudo apt-get update && sudo apt-get install -y apt-transport-https gnupg2 #bu da anahtar yakalamamizi saglayacak asagida ki komut ile(key-add)👇

curl -s https://packages.cloud.google.com/apt/doc/apt-key.gpg | sudo apt-key add - #Kubernetes icin gpg diye bir key uretecek. bu kubernetesin remote reposunun anahtaridir. apt paketimizin reposunda kubernetes yok. Dolayisiyla kubernetesin reposunu biz kendi repomuza ekleyecegiz.Ekleyebilmek icin de bu key package management ekleyecegim. package manage bunu asagida ki komutta kullaniyor 👇

echo "deb https://apt.kubernetes.io/ kubernetes-xenial main" | sudo tee -a /etc/apt/sources.list.d/kubernetes.list  #burda ki birinci bolum bir adres var ve bu adres ile curl ile baslayan yukarda ki👆 komutun icine girecek 'sudo tee -a /etc/apt/sources.list.d/kubernetes.list --binary yukleniyor
```
...
sudo apt-get install -y kubectl kubeadm kubelet kubernetes-cni docker.io # master icin
sudo apt-get install -y kubeadm kubelet kubernetes-cni docker.io         # worker icin kubectl yok
```
...
```bash 
#makinamizin request ve responselari yonlendirmesi gerekiyor.linux mak.diger makinalara trafik yonlendirmesi yapacak
...
## Bundan sonrasi master makina yuklenecek onemli
## Part 2 - Setting Up Master Node for Kubernetes
...
# Masterda yapiyoruz
```bash
sudo kubeadm config images pull # ana imagelerimizi cekiyor
```
...
```bash
sudo kubeadm init --apiserver-advertise-address=<ec2-private-ip> --pod-network-cidr=10.244.0.0/16  #kubernetes calistiracak komut # consoleda master ec2 ya git private ip al    👆 👆 yapistir.
```
...
To start using your cluster, you need to run the following as a regular user:
  ...
  ll
  cd .kube/
  ll  #config file mizin burda oldugunu gormus oluyoruz
  vi config #ile icine bakiyoruz
  cat config #yapabiliriz
...
```bash
sudo kubeadm token create --print-join-command #token tekrar almak icin
...
https://medium.com/devopsturkiye/kubernetes-giri%C5%9F-c3eefae5b9b5