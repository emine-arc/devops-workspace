...
```bash
mkdir pv-data && cd pv-data  # kube-worker makinamizda olusturuyoruz.
...
```bash  # kube-master makinamiza baglaniyoruz
...
metadata:
  ...
  labels:
    type: local
spec:
  storageClassName: manual #su anda dedigimiz hostpath
  capacity:
    storage: 5Gi
  accessModes:
    - ReadWriteOnce
  hostPath:
    path: "/home/ubuntu/pv-data" # volume baglandigi directory burasi
```
...
spec:
  storageClassName: manual # burasi pv ile ayni olmasi lazim
  accessModes:              # bununda ayni sekilde ayni olmasi lazim PV ile eger farkli ise baglanmaz
    - ReadWriteOnce
  resources:
    requests:
      storage: 5Gi
```
  👉Oneml Not : PV gore PVC duzenleyeceksiniz
...
spec:
  volumes:
    - name: clarus-pv-storage # clarus-pv-claim cagiriyoruz
...
```bash
kubectl exec -it clarus-pod -- /bin/bash #icerisine bakiyoruz
```
pwd && cd /usr/share/nginx/html && ls
...
docker ps && docker exec -it <nginx id yaz> bash
cd /usr/share/nginx/html # gidildiginde ayni index.html gor
```bash
cd pv-data
...
```bash
kubectl expose pod clarus-pod --port=80 --type=NodePort # ile yayinlayacagiz
```
...
kubectl get svc
```
kubectl get svc clarus-pod -o yaml # o service inline ile bir manifesto file olusturur
...
spec:
  ...
  accessModes:
    - ReadWriteOnce
  hostPath:
    path: "/home/ubuntu/pv-data" # mount edilen yer burasi
```
...
```bash
kubectl exec -it nginx-pod -- bash #podun icerisindeyiz
root@nginx-pod:/# ls
...
https://fatihboy.com/en/kubernetes-persistent-volume/